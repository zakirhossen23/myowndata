...
function StudyDetails() {
	...
	const { base, UpdateAudience, UpdateReward, UpdateAges, UpdateStudyTitle, CreateSubject,UpdateSubject } = useDBContext();
	...
	async function UpdateRewarads(event) {
		...
		try {
			await UpdateReward(params.id, rewardselect.value, Number(rewardprice.value.replace(" Points", "")), parseInt(totalspendlimit.value.replace(" Points", "")));
		} catch (error) {
			console.error(error);
		}
