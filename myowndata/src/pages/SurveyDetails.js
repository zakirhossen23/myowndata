...
function SurveyDetails() {
	...
	const { base,createSurveyCategory,CreateOrSaveSections } = useDBContext();
	...
	async function AddCategory(e, index) {
		...
		await createSurveyCategory(categoryname.value, categoryimagelink.value);
	...
	async function SaveSection(e) {
		...
		try {
			await CreateOrSaveSections(SURVEY_DATA.id, JSON.stringify(sectionsdata));
		} catch (error) {
	...
	async function LoadDataStudy() {
		...
		const studyRecord = await studyTable.find(location.state.studyID);
	...
	async function LoadSurveyData() {
		...
		const surveyRecord = await surveysTable.find(params.id);
	...
	async function LoadDataSections() {
		...
		try {
			...
			const records = await surveyDataTable.select({
				filterByFormula: `{survey_id} = '${params.id}'`
			}).firstPage();

			if (records.length > 0) {
				let SectionsInfo = JSON.parse(records[0].fields.sections);
	...
	async function LoadDataCategories() {
		...
		const records = await surveyCategoriesTable.select().all();

		let category = [];
		for (let i = 0; i < Number(records.length); i++) {
			const element = records[i].fields;
			category.push({
				value: element.name,
				text: element.name,
