...
export default function CreateStudyModal({show, onHide}) {
    const {CreateStudy} = useDBContext();
	...
	async function CreateStudyHandle(e) {
		...
		try {
			await CreateStudy(Number(Cookies.get("userid")),image.value,title.value,description.value, JSON.stringify(permissionData), 0,0,0);
